...
for receiver_index=1:n_receivers
    %     time_matrix_per_receiver=time_matrix{receiver_index};
%     correlation_val_per_receiver=correlation_val{receiver_index};
    ...
    dir_val_widir=zeros(n_antennas,n_subcarriers_selected);
    for sample_index=1:n_sample
        ...
        for jj=1:n_antennas
            dir_val_widir(jj,:)=dir_val(jj,jj,:);
        end
        node_sequence_per_receiver(sample_index)=mode(dir_val_widir,'all')+2;    
        %         end
    end
